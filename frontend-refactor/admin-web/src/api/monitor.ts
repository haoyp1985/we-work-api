...
export const monitorApi = {
   ...
   * 获取单个服务健康状态
   */
  getServiceHealthById(serviceName: string): Promise<ApiResponse<ServiceHealth>> {
   ...
   * 测试告警规则
   */
  testAlertRule(id: string): Promise<ApiResponse<{
    triggered: boolean
    message?: string
    conditions: any[]
  }>> {
   ...
   * 批量处理告警
   */
  batchProcessAlerts(ids: string[], action: 'ACKNOWLEDGE' | 'RESOLVE', data?: any): Promise<ApiResponse<void>> {
    return request.post('/monitor/alert-records/batch-process', { ids, action, ...data })
  },
   ...
   * 获取实时性能数据
   */
  getRealTimePerformanceData(serviceName?: string): Promise<ApiResponse<PerformanceData>> {
   ...
   * 获取日志条目
   */
  getLogEntries(params: LogSearchForm): Promise<ApiResponse<PageResult<LogEntry>>> {
   ...
   * 导出日志
   */
  exportLogs(params: LogSearchForm & {
    format: 'CSV' | 'JSON' | 'TXT'
   ...
   * 测试告警通道
   */
  testAlertChannel(id: string, message?: string): Promise<ApiResponse<{
    success: boolean
    responseTime?: number
    errorMessage?: string
  }>> {
   ...
   * 测试外部集成连接
   */
  testExternalIntegration(id: string): Promise<ApiResponse<{
    success: boolean
    responseTime?: number
    errorMessage?: string
  }>> {
   ...
   * 获取系统状态页面
   */
  getStatusPage(): Promise<ApiResponse<{
