...
export const useMonitorStore = defineStore('monitor', () => {
  ...
  const criticalAlerts = computed(() => 
    alertRecords.value.filter(alert => alert.severity === 'CRITICAL' && alert.status === 'ACTIVE')
  )
   ...
   * 获取性能数据
   */
  const fetchPerformanceData = async (params?: any): Promise<PerformanceData[]> => {
    try {
   ...
   * 更新告警规则
   */
  const updateAlertRule = async (id: string, ruleData: any): Promise<AlertRule> => {
    try {
      ...
      if (response.code === 200) {
        ...
        if (index !== -1) {
          alertRules.value[index] = { ...alertRules.value[index], ...response.data }
        }
        return response.data
      } else {
   ...
   * 解决告警
   */
  const resolveAlert = async (id: string, resolution?: string): Promise<void> => {
    try {
