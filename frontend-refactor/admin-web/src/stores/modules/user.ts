...
export const useUserStore = defineStore('user', () => {
  ...
  const roleCodes = computed(() => 
    roles.value.map(role => role.roleCode)
  )

  const isAdmin = computed(() => 
    roleCodes.value.includes('SUPER_ADMIN') || roleCodes.value.includes('TENANT_ADMIN')
  )

  const menuPermissions = computed(() => 
    permissions.value.filter(permission => permission.permissionType === 'MENU')
  )

  const buttonPermissions = computed(() => 
    permissions.value.filter(permission => permission.permissionType === 'BUTTON')
  )
  ...
  const login = async (loginForm: LoginForm): Promise<LoginResponse> => {
    try {
      ...
      if (response.code === 200) {
        const { token: newToken, userInfo: userData, roles: userRoles, permissions: userPermissions } = response.data
  ...
  const getUserInfo = async (): Promise<User> => {
    try {
      ...
      if (response.code === 200) {
        const { userInfo: userData, roles: userRoles, permissions: userPermissions } = response.data
  ...
  const hasAnyPermission = (permissionCodeList: string[]): boolean => {
    ...
    return permissionCodeList.some(code => permissionCodes.value.includes(code))
  }
