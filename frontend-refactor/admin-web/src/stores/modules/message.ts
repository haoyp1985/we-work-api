...
export const useMessageStore = defineStore('message', () => {
   ...
   * 获取消息列表
   */
  const fetchMessageList = async (params?: Partial<MessageSearchForm>): Promise<PageResult<Message>> => {
    loading.value = true
    try {
   ...
   * 获取消息模板列表
   */
  const fetchTemplateList = async (params?: any): Promise<PageResult<MessageTemplate>> => {
    loading.value = true
    try {
   ...
   * 发送消息
   */
  const sendMessage = async (messageData: MessageSendForm): Promise<Message> => {
   ...
   * 创建消息模板
   */
  const createTemplate = async (templateData: any): Promise<MessageTemplate> => {
    try {
   ...
   * 更新消息模板
   */
  const updateTemplate = async (id: string, templateData: any): Promise<MessageTemplate> => {
    try {
      ...
      if (response.code === 200) {
        ...
        if (index !== -1) {
          templateList.value[index] = { ...templateList.value[index], ...response.data }
        }
        
        // 更新当前模板
        if (currentTemplate.value && currentTemplate.value.id === id) {
          currentTemplate.value = { ...currentTemplate.value, ...response.data }
        }
   ...
   * 搜索消息
   */
  const searchMessages = async (searchParams: Partial<MessageSearchForm>): Promise<void> => {
   ...
   * 获取批次列表
   */
  const fetchBatchList = async (params?: any): Promise<PageResult<MessageBatch>> => {
    loading.value = true
    try {
